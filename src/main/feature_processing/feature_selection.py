...
def select_features(train_x, train_y, test_x, test_y, selection_type, num_features=20, verbose=False):
    # This function orders feature from Mutual Info, selects top 60 of the
    # feature and the performs RFE on the selected features.

    if selection_type == "classification":

        # Feature Selection through Mutual Information. Getting refined train and test splits.
        mi = feature_selection_mutualinfo_classif(train_x, train_y, verbose=verbose)
    ...
    if verbose:
        print("Feature Seletion Support: \n", selector.support_)
